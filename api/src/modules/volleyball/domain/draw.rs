...
pub fn draw<R: Rng>(
    ...
    let after_cooldown = apply_cooldown(eligible, cooldown, n);
    let tolerance_group = build_tolerance_group(&after_cooldown, n);
    let selected = random_select(rng, &tolerance_group, n)?;

    pair_by_wins(&selected, n)
}
...
mod tests {
    ...
    fn apply_cooldown_excludes_normally() {
        ...
        let result = apply_cooldown(&candidates, &cooldown, 2);

        assert_eq!(result.len(), 2);
        assert!(!result.iter().any(|c| c.player_id == candidates[0].player_id));
    }
    ...
    fn tolerance_group_keeps_min_and_min_plus_one() {
        ...
        let group = build_tolerance_group(&candidates, 2);
    ...
    fn tolerance_group_expands_window_when_not_enough_candidates() {
        ...
        let group = build_tolerance_group(&candidates, 3);
    ...
    fn random_select_errors_when_group_too_small() {
        ...
        let result = random_select(&mut rng, &group, 2);
    ...
    fn random_select_returns_exactly_n_distinct_players() {
        ...
        let selected = random_select(&mut rng, &group, 2).unwrap();
    ...
    fn pair_by_wins_n2_is_trivial_pair() {
        ...
        let pairs = pair_by_wins(&selected, 2).unwrap();
    ...
    fn pair_by_wins_n4_pairs_extremes_together() {
        ...
        let pairs = pair_by_wins(&selected, 4).unwrap();
    ...
    fn pair_by_wins_rejects_unsupported_n() {
        ...
        let result = pair_by_wins(&selected, 3);
    ...
    fn draw_end_to_end_produces_disjoint_balanced_pairs() {
        ...
        let pairs = draw(&mut rng, &eligible, &cooldown, 4).unwrap();
