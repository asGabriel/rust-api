...
use crate::modules::{
    ...
    telegram_bot::{self, TelegramBotState},
};

#[derive(Clone)]
pub struct AppState {
    pub finance_manager_state: Arc<FinanceManagerState>,
    pub telegram_bot_state: Arc<TelegramBotState>,
}

pub fn configure_services() -> Router<AppState> {
    let finance_manager_routes = finance_manager::routes::payment::configure_routes();
    let telegram_bot_routes = telegram_bot::routes::configure_routes();
